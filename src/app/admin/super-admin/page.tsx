...
function SuperAdminApplicationsTab() {
  ...
  const pageApplications = visibleApplications.slice(
  ...
  const statusFor = (application: SuperAdminApplication) => application.redirection ? "Redirected" : application.hasAccepted ? "Accepted" : application.status === "failed" ? "Rejected" : application.status === "evaluating" ? "Evaluating" : (application.type === "committee" || application.type === "executive-associate") && (!application.interviewSlotDay || !application.interviewSlotTimeStart) ? "Needs Schedule" : "Pending";
  const resetSchedule = async (application: SuperAdminApplication) => {
    if (!window.confirm(`Reset ${application.user.name}'s interview schedule? Their application will remain unchanged.`)) return;
    try {
      const response = await fetch("/api/admin/applications/reset-schedule", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ applicationId: application.id, type: application.type }),
      });
      const result = await response.json();
  ...
  const downloadFile = async (url: string) => {
    try {
      ...
      const result = (await response.json()) as { downloadUrl?: string; fileName?: string; error?: string };
      if (!response.ok || !result.downloadUrl) throw new Error(result.error || "File could not be downloaded");
      const fileResponse = await fetch(result.downloadUrl);
      if (!fileResponse.ok) throw new Error("File could not be downloaded");
      const blobUrl = URL.createObjectURL(await fileResponse.blob());
      const link = document.createElement("a");
      link.href = blobUrl;
      link.download = result.fileName || "application-file";
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(blobUrl);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "File could not be downloaded");
    }
  };
  ...
  return <div className="space-y-5">
    ...
    <div className="hidden min-w-0 overflow-hidden rounded-xl border border-[#005FD9]/10 bg-white md:block">
      <table className="w-full min-w-0 table-fixed text-left text-xs"><colgroup><col className="w-[24%]" /><col className="w-[18%]" /><col className="w-[10%]" /><col className="w-[16%]" /><col className="w-[12%]" /><col className="w-[20%]" /></colgroup><thead className="bg-[#134687] text-[10px] uppercase tracking-widest text-white/80"><tr><th className="px-4 py-3">Applicant</th><th className="px-4 py-3">Type / Position</th><th className="px-4 py-3 text-center">Status</th><th className="px-4 py-3 text-center">Interviewer</th><th className="px-4 py-3 text-center">Files</th><th className="px-4 py-3 text-center">Actions</th></tr></thead><tbody className="divide-y divide-[#005FD9]/10">
        {loading ? <tr><td colSpan={6} className="px-4 py-10 text-center text-[#134687]/50">Loading applications...</td></tr> : pageApplications.map((application) => <tr key={`${application.type}:${application.id}`} className="align-middle hover:bg-[#F9FBFF]"><td className="min-w-0 break-all px-4 py-3"><div className="font-semibold text-[#134687]">{application.user.name}</div><div className="text-xs text-[#134687]/60">{application.user.email}</div><div className="font-mono text-xs text-[#134687]/50">{application.user.studentNumber}</div></td><td className="break-words px-4 py-3 text-[#134687]">{labelFor(application)}</td><td className="px-4 py-3 text-center"><span className="inline-flex rounded-full bg-[#E8F2FF] px-2.5 py-1 text-xs font-semibold text-[#044FAF]">{statusFor(application)}</span></td><td className="break-words px-4 py-3 text-center text-xs text-[#134687]/70"><div>{application.interviewBy || "Not assigned"}</div>{application.interviewSlotDay && application.interviewSlotTimeStart && <div className="mt-1 text-[11px] text-[#134687]/55">{application.interviewSlotDay} at {application.interviewSlotTimeStart}{application.interviewSlotTimeEnd ? ` - ${application.interviewSlotTimeEnd}` : ""}</div>}</td><td className="px-4 py-3"><div className="flex items-center justify-center gap-1.5">{application.cvDownloadUrl && <button type="button" onClick={() => void downloadFile(application.cvDownloadUrl!)} className="inline-flex min-w-10 items-center justify-center rounded-md border border-[#005FD9]/15 bg-[#F3F8FF] px-2 py-1 text-xs font-semibold text-[#044FAF] transition-colors hover:bg-[#E8F2FF]">CV</button>}{application.portfolioDownloadUrl && <button type="button" onClick={() => void downloadFile(application.portfolioDownloadUrl!)} className="inline-flex items-center justify-center rounded-md border border-[#005FD9]/15 bg-[#F3F8FF] px-2 py-1 text-xs font-semibold text-[#044FAF] transition-colors hover:bg-[#E8F2FF]">Portfolio</button>}</div></td><td className="px-4 py-3"><div className="flex flex-wrap justify-center gap-1"><button onClick={() => setPending({ application, action: "evaluate" })} disabled={application.type === "member" || Boolean(application.hasAccepted)} className="rounded border border-[#005FD9]/15 px-2 py-1 text-xs text-[#134687] disabled:opacity-30">Evaluate</button><button onClick={() => setPending({ application, action: "accept" })} disabled={Boolean(application.hasAccepted)} className="rounded border border-[#005FD9]/15 px-2 py-1 text-xs text-[#134687] disabled:opacity-30">Accept</button><button onClick={() => setPending({ application, action: "reject" })} disabled={Boolean(application.hasAccepted)} className="rounded border border-red-200 px-2 py-1 text-xs text-red-600 disabled:opacity-30">Reject</button><button onClick={() => setPending({ application, action: "redirect" })} disabled={application.type === "member" || Boolean(application.hasAccepted)} className="rounded border border-[#005FD9]/15 px-2 py-1 text-xs text-[#134687] disabled:opacity-30">Redirect</button><button onClick={() => setPending({ application, action: "reset" })} className="rounded border border-red-200 px-2 py-1 text-xs text-red-600">Reset</button>{(application.type === "committee" || application.type === "executive-associate") && application.interviewSlotDay && application.interviewSlotTimeStart && <button onClick={() => void resetSchedule(application)} className="rounded border border-[#D6A326]/30 px-2 py-1 text-xs text-[#8A5A00] hover:bg-[#FFF8E7]">Reset Schedule</button>}</div></td></tr>)}
    ...
    <div className="space-y-3 md:hidden">
      {loading ? <div className="rounded-xl border border-[#005FD9]/10 bg-white px-4 py-10 text-center text-xs text-[#134687]/50">Loading applications...</div> : pageApplications.map((application) => (
        <article key={`mobile:${application.type}:${application.id}`} className="rounded-xl border border-[#005FD9]/10 bg-white p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3 className="truncate text-sm font-semibold text-[#134687]">{application.user.name}</h3>
              <p className="break-all text-[11px] text-[#134687]/60">{application.user.email}</p>
              <p className="font-mono text-[11px] text-[#134687]/50">{application.user.studentNumber}</p>
            </div>
            <span className="shrink-0 rounded-full bg-[#E8F2FF] px-2 py-1 text-[10px] font-semibold text-[#044FAF]">{statusFor(application)}</span>
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            ...
            {application.portfolioDownloadUrl && <button type="button" onClick={() => void downloadFile(application.portfolioDownloadUrl!)} className="rounded-md border border-[#005FD9]/15 bg-[#F3F8FF] px-2.5 py-1 text-xs font-semibold text-[#044FAF]">Portfolio</button>}
          </div>
          <div className="mt-3 flex flex-wrap gap-1.5 border-t border-[#005FD9]/10 pt-3">
            <button onClick={() => setPending({ application, action: "evaluate" })} disabled={application.type === "member" || Boolean(application.hasAccepted)} className="rounded border border-[#005FD9]/15 px-2 py-1 text-xs text-[#134687] disabled:opacity-30">Evaluate</button>
            <button onClick={() => setPending({ application, action: "accept" })} disabled={Boolean(application.hasAccepted)} className="rounded border border-[#005FD9]/15 px-2 py-1 text-xs text-[#134687] disabled:opacity-30">Accept</button>
            <button onClick={() => setPending({ application, action: "reject" })} disabled={Boolean(application.hasAccepted)} className="rounded border border-red-200 px-2 py-1 text-xs text-red-600 disabled:opacity-30">Reject</button>
            <button onClick={() => setPending({ application, action: "redirect" })} disabled={application.type === "member" || Boolean(application.hasAccepted)} className="rounded border border-[#005FD9]/15 px-2 py-1 text-xs text-[#134687] disabled:opacity-30">Redirect</button>
            <button onClick={() => setPending({ application, action: "reset" })} className="rounded border border-red-200 px-2 py-1 text-xs text-red-600">Reset</button>
            {(application.type === "committee" || application.type === "executive-associate") && application.interviewSlotDay && application.interviewSlotTimeStart && <button onClick={() => void resetSchedule(application)} className="rounded border border-[#D6A326]/30 px-2 py-1 text-xs text-[#8A5A00]">Reset Schedule</button>}
          </div>
        </article>
      ))}
      {!loading && pageApplications.length === 0 && <div className="rounded-xl border border-[#005FD9]/10 bg-white px-4 py-10 text-center text-xs text-[#134687]/50">No applications found.</div>}
    </div>
    <div className="flex flex-col gap-3 rounded-xl border border-[#005FD9]/10 bg-white px-4 py-3 text-xs text-[#134687]/60 sm:flex-row sm:items-center sm:justify-between">
      <span>{visibleApplications.length === 0 ? "No applications" : `Showing ${(currentPage - 1) * pageSize + 1}-${Math.min(currentPage * pageSize, visibleApplications.length)} of ${visibleApplications.length}`}</span>
      <div className="flex gap-1">
        <button type="button" onClick={() => setCurrentPage((page) => Math.max(1, page - 1))} disabled={currentPage === 1} className="rounded-lg border border-[#005FD9]/15 px-3 py-1.5 font-medium text-[#134687] hover:bg-[#F3F8FF] disabled:cursor-not-allowed disabled:opacity-35">Previous</button>
        <span className="px-3 py-1.5 font-mono text-[#134687]">{currentPage} / {totalPages}</span>
        <button type="button" onClick={() => setCurrentPage((page) => Math.min(totalPages, page + 1))} disabled={currentPage === totalPages} className="rounded-lg border border-[#005FD9]/15 px-3 py-1.5 font-medium text-[#134687] hover:bg-[#F3F8FF] disabled:cursor-not-allowed disabled:opacity-35">Next</button>
      </div>
    </div>
...
export default function SuperAdminDashboard() {
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-[#F3F3FD] bg-[url('/assets/css-apply-static-images/assets/pictures/background.webp')] bg-cover bg-repeat">
      ...
      <main className="min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain px-6 pb-6 [scrollbar-gutter:stable] md:px-8 md:pb-8">
