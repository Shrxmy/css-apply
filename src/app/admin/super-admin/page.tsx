...
function SuperAdminApplicationsTab() {
  ...
  return <div className="space-y-5">
    ...
    <div className="overflow-x-auto rounded-xl border border-[#005FD9]/10 bg-white">
      <table className="min-w-[1200px] w-full table-fixed text-left text-sm"><colgroup><col className="w-[24%]" /><col className="w-[18%]" /><col className="w-[10%]" /><col className="w-[16%]" /><col className="w-[12%]" /><col className="w-[20%]" /></colgroup><thead className="bg-[#134687] text-[10px] uppercase tracking-widest text-white/80"><tr><th className="px-4 py-3">Applicant</th><th className="px-4 py-3">Type / Position</th><th className="px-4 py-3 text-center">Status</th><th className="px-4 py-3 text-center">Interviewer</th><th className="px-4 py-3 text-center">Files</th><th className="px-4 py-3 text-center">Actions</th></tr></thead><tbody className="divide-y divide-[#005FD9]/10">
        {loading ? <tr><td colSpan={6} className="px-4 py-10 text-center text-[#134687]/50">Loading applications...</td></tr> : pageApplications.map((application) => <tr key={`${application.type}:${application.id}`} className="align-middle hover:bg-[#F9FBFF]"><td className="px-4 py-3"><div className="font-semibold text-[#134687]">{application.user.name}</div><div className="text-xs text-[#134687]/60">{application.user.email}</div><div className="font-mono text-xs text-[#134687]/50">{application.user.studentNumber}</div></td><td className="px-4 py-3 text-[#134687]">{labelFor(application)}</td><td className="px-4 py-3 text-center"><span className="inline-flex rounded-full bg-[#E8F2FF] px-2.5 py-1 text-[10px] font-semibold text-[#044FAF]">{statusFor(application)}</span></td><td className="px-4 py-3 text-center text-xs text-[#134687]/70">{application.interviewBy || "Not assigned"}</td><td className="px-4 py-3"><div className="flex items-center justify-center gap-1.5">{application.cvDownloadUrl && <a href={application.cvDownloadUrl} target="_blank" rel="noreferrer" className="inline-flex min-w-10 items-center justify-center rounded-md border border-[#005FD9]/15 bg-[#F3F8FF] px-2.5 py-1 text-[11px] font-semibold text-[#044FAF] transition-colors hover:bg-[#E8F2FF]">CV</a>}{application.portfolioDownloadUrl && <a href={application.portfolioDownloadUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center rounded-md border border-[#005FD9]/15 bg-[#F3F8FF] px-2.5 py-1 text-[11px] font-semibold text-[#044FAF] transition-colors hover:bg-[#E8F2FF]">Portfolio</a>}</div></td><td className="px-4 py-3"><div className="flex min-w-[360px] flex-wrap justify-center gap-1.5"><button onClick={() => setPending({ application, action: "evaluate" })} disabled={application.type === "member" || Boolean(application.hasAccepted)} className="rounded border border-[#005FD9]/15 px-2 py-1 text-xs text-[#134687] disabled:opacity-30">Evaluate</button><button onClick={() => setPending({ application, action: "accept" })} disabled={Boolean(application.hasAccepted)} className="rounded border border-[#005FD9]/15 px-2 py-1 text-xs text-[#134687] disabled:opacity-30">Accept</button><button onClick={() => setPending({ application, action: "reject" })} disabled={Boolean(application.hasAccepted)} className="rounded border border-red-200 px-2 py-1 text-xs text-red-600 disabled:opacity-30">Reject</button><button onClick={() => setPending({ application, action: "redirect" })} disabled={application.type === "member" || Boolean(application.hasAccepted)} className="rounded border border-[#005FD9]/15 px-2 py-1 text-xs text-[#134687] disabled:opacity-30">Redirect</button><button onClick={() => setPending({ application, action: "reset" })} className="rounded border border-red-200 px-2 py-1 text-xs text-red-600">Reset</button></div></td></tr>)}
